...
class NeuroProcessor:
    ...
    def _initialize_models(
        ...
        try:
            ...
            if (
                ...
                and os.path.exists(transformer_model_path)
                and TORCH_AVAILABLE
            ):
        ...
        try:
            if TORCH_AVAILABLE:
                feature_dim = (
                    ...
                    if self.transformer
                    else len(self.electrode_names) * 5
                )  # Rough estimate
                ...
                if classifier_model_path and os.path.exists(classifier_model_path):
                    ...
                    logger.info(
                        f"Loaded classifier weights from {classifier_model_path}"
                    )
            else:
                self.classifier = MotorImageryClassifier()
    ...
    def _generate_simulated_data(self) -> np.ndarray:
        ...
        if self.simulated_class == "left_hand":
            ...
            c4_idx = (
                self.electrode_names.index("C4") if "C4" in self.electrode_names else 0
            )
            ...
            c3_idx = (
                self.electrode_names.index("C3") if "C3" in self.electrode_names else 0
            )
            ...
            cz_idx = (
                self.electrode_names.index("Cz") if "Cz" in self.electrode_names else 0
            )
    ...
    def _process_data_chunk(self, data_chunk: np.ndarray) -> None:
        ...
        feature_mat = np.array(
            [self.features[f"band_{band}"] for band in self.frequency_bands.keys()]
        )
        ...
        if self.transformer is not None:
            ...
            if TORCH_AVAILABLE:
                ...
                self.transformer_output, self.attention_weights = self.transformer(
                    feature_mat, return_attention=True
                )
        else:
            ...
            if TORCH_AVAILABLE:
                ...
                if self.using_simulated_data:
                    ...
                    self.classification = {
                        ...
                        "confidence": confidence,
                        "probabilities": {
                            cls: probs[i] for i, cls in enumerate(self.classes)
                        },
                        "timestamp": time.time(),
                    }
                else:
    ...
    def set_simulated_class(self, class_name: str, confidence: float = 0.8) -> None:
        ...
        if class_name in self.classes:
            ...
            logger.info(
                f"Set simulated class to {class_name} with confidence {confidence:.2f}"
            )
        else:
            logger.warning(
                f"Invalid class name: {class_name}. Valid classes: {self.classes}"
            )
        ...
        if "sampling_rate" in settings:
            ...
            if new_rate != self.sampling_rate:
                ...
                self.data_buffer = np.zeros(
                    (len(self.electrode_names), self.buffer_samples)
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "simulated_class": (
                self.simulated_class if self.using_simulated_data else None
            ),
            "simulated_confidence": (
                self.simulated_confidence if self.using_simulated_data else None
            ),
