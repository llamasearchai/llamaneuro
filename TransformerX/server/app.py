...
try:
    ...
    HAS_FLASK = False
    logger.error(
        "Flask or Flask-CORS not available, please install with: pip install flask flask-cors"
    )
    sys.exit(1)
...
try:
    ...
    from llama_interface import HAS_LLAMA, LlamaInterface
...
def create_app(config_path=None):
    ...
    dashboard_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), dashboard_dir)
    )
...
def main():
    ...
    logger.info(
        f"API available at http://{host}:{port}{config.get('server.api_prefix')}"
    )
