...
def test_components():
    ...
    try:
        processor = NeuroProcessor(
            sampling_rate=250, buffer_duration=2.0, using_simulated_data=True
        )
    ...
    try:
        llama = LlamaInterface(model_path="dummy_model", use_quantization=False)
...
def test_api(base_url: str = "http://localhost:8080"):
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"Neuro start endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"Neuro status endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"Neuro simulate endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"LLaMA start endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"LLaMA status endpoint returned {response.status_code}"
        data = response.json()
        assert "active" in data, "No active status in response"
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"LLaMA generate endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"Neural LLaMA endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"Neuro stop endpoint returned {response.status_code}"
        data = response.json()
    ...
    try:
        ...
        assert (
            response.status_code == 200
        ), f"LLaMA stop endpoint returned {response.status_code}"
        data = response.json()
...
def main():
    ...
    parser.add_argument(
        "--components", action="store_true", help="Test individual components"
    )
    parser.add_argument("--api", action="store_true", help="Test API endpoints")
    parser.add_argument(
        "--url", type=str, default="http://localhost:8080", help="Server URL"
    )
